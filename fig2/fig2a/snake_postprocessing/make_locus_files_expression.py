...
def make_gene_file(gene,trait,tissue):
    ...
    if data_source=="CMC":
        out.write("gene\trsid\teQTL_ref\tTSS\tgenom_distance\tMAF\tP\tt\n")
        ### Get TSS
        ...
        for ln in eQTL_dat:
            rsid,eQTL_gene,slope,t,pval,FDR = ln.split()[:6]
            if eQTL_gene==gene:
                ...
                if MAF != '.':
                    ...
                    if genom_dist != '.':
                        newln = "\t".join([gene, rsid, ref, str(TSS), str(genom_dist), str(MAF), str(pval), str(t)]) + "\n"
                        out.write(newln)
