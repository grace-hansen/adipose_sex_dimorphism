...
count_genos<-function(gene_dat,rsid,pheno) {
  ...
  geno_exp$Expression<-as.numeric(geno_exp$Expression)
  return(geno_exp)
}
...
if (data_source=="GTEx_v8") {
  ...
  system(cmd)
  cmd=paste("zcat ../../",tissue,"_",data_source,"_exp_1.txt.gz | awk '{ if ($3 == \"",gene,"\") print $0}' >> ",gene,"_pheno",sep='')
  system(cmd)
}
...
geno_exp<-count_genos(gene_dat,rsid,pheno)
